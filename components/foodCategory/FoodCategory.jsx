...
import { clearFood, removeCategory, selectCategory } from "../../app/slices/foodSlice";
...
const CategoryItem = ({ item, onPress, activeCategory }) => {
	return (
		...
		>
			...
			<Text style={[styles.categoryTitle, activeCategory === item.id && { color: "#fff", fontWeight: "bold" }]}>
				{item.type}
			</Text>
		</Pressable>
	);
...
const FoodCategory = ({ refreshState }) => {
	const [getFoodTypeList, { isError, data: foodTypeList = [], isLoading }] = useGetFoodTypeListMutation();
		...
		<View style={styles.container}>
			{!isLoading &&
				foodTypeList.map((item, index) => {
					return <CategoryItem key={index} item={item} onPress={handleActiveCategory} activeCategory={categoryType} />;
				})}
