...
import { useAddFoodMutation, useEditFoodQuery } from "../../../app/services/foodListApi";
...
const EditFood = ({ navigation, route }) => {
	...
	} = useEditFoodQuery({ id: foodId });

	useEffect(() => {
		if (isFoodSuccess) {
			setIsSwitchOn(foodData.FoodDetails.status);
			setIngredients(foodData.IngredientDetails);
			setImage({ uri: foodData.FoodDetails.image_url });
			setValue(foodData.FoodDetails.food_type_id);
			formik.setFieldValue("foodType", foodData.FoodDetails.food_type_id);
		}
	}, [isFoodSuccess]);
	...
	const formik = useFormik({
		initialValues: {
			foodName: foodData?.FoodDetails.food_name || "",
			foodType: foodData?.FoodDetails.food_type_id || "",
			price: foodData?.FoodDetails.price || "",
			description: foodData?.FoodDetails.short_description || "",
		},
	...
	const handleAddFood = async (credential, setSubmitting) => {
		...
		formData.append("id", JSON.stringify(foodId));
		addFood(formData);
	...
	useEffect(() => {
		if (isSuccess) navigation.navigate(Routes.cook.food);
	}, [isSuccess]);

	return (
		<ScrollView style={[GlobalStyles.container]}>
			{isFoodSuccess && !isFoodLoading && (
